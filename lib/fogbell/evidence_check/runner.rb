  ...
  module EvidenceCheck
    ...
    class Runner
      ...
      def initialize(check, llm: EvidenceCheck.llm, rulebook: Rulebook.instance, manifest: Review::Manifest.default,
                     template: Pipeline::PromptTemplate.new(Rails.root.join(TEMPLATE)), log_dir: Rails.root.join(Rails.env.test? ? "tmp/evidence_checks" : Rails.env.production? ? "storage/evidence_checks" : "log/evidence_checks"))
        @check = check
        @llm = llm
        @rulebook = rulebook
        @manifest = manifest
